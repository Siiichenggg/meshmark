...
<sub>一间手术室的摄影测量扫描，标注进行中。黄色：画在物体上的包围盒。红色：参考文件声称的位置。青色：在地面上画出的一条路线。右上：同一个物体的正上方视图，6.2 毫米/像素。</sub>
...
两个视图编辑的是同一份标注：在 3D 里点击网格放下包围盒，再到俯视图上用方向键
一次 1 厘米地微调。
...
- **它不做任何自动分割。** 它按设计就是手动标定器。摄影测量扫描通常是整个房间
  蒙一层焊死的连续曲面，没有任何聚类方法能把推车和它背后的墙分开。
- **它不测量高度。** 宽、深、朝向是你拖到物体上的；高度从类别默认值开始，
  除非你手填，否则不会变。导出里的 `height_source` 会写明这个高度是哪一种。
...
| `box.height_source` | `class default` 或 `entered by hand` |
...
早期阶段 —— 0.1.0。上面这些格式都带版本号，所以一旦有破坏性改动，它会自己声明。
