...
- **它不替你测高度。** 宽、深、朝向是你拖到物体上的，高度现在也可以 ——
  框顶的菱形沿一条竖直导轨往上拉，拉到框刚好罩住物体为止。但这件事不会自己发生：
  没人碰过的高度就是类别默认值，那是一个猜测。导出里的 `height_source` 会写明
  这个高度是三者中的哪一种 —— 对着网格量出来的数，和从来没人看过的数，
  不该被同样地引用。
...
早期阶段 —— 0.2.0。上面这些格式都带版本号，所以一旦有破坏性改动，它会自己声明。
